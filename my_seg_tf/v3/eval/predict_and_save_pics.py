...
import sys
sys.path.append('../')
from model.SegCaps_v2 import SegCaps
...
if  __name__== '__main__':
    ...
    with tf.Session(config=session_config) as sess:
        ...
        for i in range(test_data_number//batch_size):
            ...
            for img, label, predict,recon in zip(pics, pics_masks, pres,recons):
                img= img*255
                label = label*255
                predict = predict*255
                recon = recon*255
                ...
                hstack_1 = np.hstack((img, label))
                hstack_2 = np.hstack((predict, recon))
                vstack_3 = np.vstack((hstack_1, hstack_2))
                vstack_4 = cv2.resize(vstack_3,(512,512))
                save_name = '{}.jpg'.format(index)
                save_name = os.path.join(predict_pics_save, save_name)
                cv2.imwrite(save_name, vstack_4)
                index += 1
                print(save_name)
