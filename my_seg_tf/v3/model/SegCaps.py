...
class SegCaps(object):
    ...
    def build(self, images):
        ...
        # 1/2
        x = self.capsule(x, "conv", k=3, s=2, t=2, z=16, routing=1)
        x = self.capsule(x, "conv", k=3, s=1, t=4, z=16, routing=3)
        skip2 = x

        # 1/4
        x = self.capsule(x, "conv", k=3, s=2, t=4, z=32, routing=3)
        x = self.capsule(x, "conv", k=3, s=1, t=8, z=32, routing=3)
        skip3 = x

        # 1/8
        x = self.capsule(x, "conv", k=3, s=2, t=8, z=64, routing=3)
        x = self.capsule(x, "conv", k=3, s=1, t=8, z=32, routing=3)
