...
class SegCaps(object):
    def __init__(self, sess, config, is_train):
        ...
        # self.acc = self.compute_acc(self.v_lens,self.labels )
    ...
    # def compute_acc(self,predict,labels ):
    #     predict = np.atleast_1d(predict.astype(np.bool))
    #     labels = np.atleast_1d(labels.astype(np.bool))
    #
    #     intersection = np.count_nonzero(predict & labels)
    #     union = np.count_nonzero(predict | labels)
    #
    #     jc = float(intersection) / float(union)
    #     self.acc_summaries = [
    #         tf.summary.scalar("acc", jc)]
    #     return jc
