"""
站在capsnet_em_v1的基础上（multi-scale）改进：

用conv1 output对primary capsule做Identity mapping
conv1 output shape: (32, 12, 12, 32)  改成 (32, 12, 12, 128)
primary capsule output shape: (32, 12, 12, 136)
然后 conv1 output +  primary capsule output  得 (32, 12, 12, 136)
"""
...
def build_arch(input, coord_add, is_train: bool, num_classes: int):
    input = print_tensor(input, message='input')
    # 不管训练还是测试必须要设置is_training=True，否则识别不正常
    input = slim.batch_norm(input, center=True, is_training=True, trainable=False)
    input = print_tensor(input, message='batch_x_norm')
    ...
    with slim.arg_scope([slim.conv2d], trainable=is_train, biases_initializer=bias_initializer, weights_regularizer=weights_regularizer):
        with tf.variable_scope('relu_conv1') as scope:
            ...
            #                      3, 3], stride=2, padding='VALID', scope=scope, activation_fn=tf.nn.relu)
            # output = slim.conv2d(output, num_outputs=cfg.A, kernel_size=[
            #     3, 3], stride=1, padding='VALID', scope=scope, activation_fn=tf.nn.relu)
            branch_0 = slim.max_pool2d(input, [2, 2], stride=2, padding='VALID', scope='MaxPool')
            branch_0 = slim.conv2d(branch_0, num_outputs=cfg.A /4,
                padding='VALID', kernel_size=[3, 3],stride=1,scope='branch_0',activation_fn=tf.nn.relu)

            branch_1 = slim.conv2d(input, num_outputs=cfg.A /4, kernel_size=[
                3, 3], stride=1, padding='VALID', scope='branch_1_1', activation_fn=tf.nn.relu)

            branch_1 = slim.conv2d(branch_1, num_outputs=cfg.A /4, kernel_size=[
                3, 3], stride=2, padding='VALID', scope='branch_1_2', activation_fn=tf.nn.relu)

            branch_2 = slim.conv2d(input, num_outputs=cfg.A /4, kernel_size=[
                5, 5], stride=2, padding='VALID', scope='branch_2', activation_fn=tf.nn.relu)

            branch_3 = tf.image.resize_images(input, [12, 12], method=1)
            branch_3 = slim.conv2d(branch_3, num_outputs=cfg.A /4, kernel_size=[
                1, 1], stride=1, padding='VALID', scope='branch_3', activation_fn=tf.nn.relu)

            data_size = int(np.floor((data_size - 4) / 2))
            output_relu_conv1 = tf.concat([branch_0, branch_1, branch_2, branch_3], 3)

            assert output_relu_conv1.get_shape() == [cfg.batch_size, data_size, data_size, cfg.A]
            tf.logging.info('conv1 output shape: {}'.format(output_relu_conv1.get_shape()))

        with tf.variable_scope('primary_caps') as scope:
            pose = slim.conv2d(output_relu_conv1, num_outputs=cfg.B * 16,
                               kernel_size=[1, 1], stride=1, padding='VALID', scope=scope, activation_fn=None)
            activation = slim.conv2d(output_relu_conv1, num_outputs=cfg.B, kernel_size=[
            ...
            output_primary_caps = tf.reshape(output, shape=[cfg.batch_size, data_size, data_size, -1])
            assert output_primary_caps.get_shape() == [cfg.batch_size, data_size, data_size, cfg.B * 17]
            tf.logging.info('primary capsule output shape: {}'.format(output_primary_caps.get_shape()))

            Identity_output = output_relu_conv1 + output_primary_caps


        with tf.variable_scope('conv_caps1') as scope:
            output = kernel_tile(Identity_output, 3, 2)
    ...
    output1 = tf.reshape(output, shape=[cfg.batch_size, num_classes])
    # result = tf.argmax(output1, axis=-1)
