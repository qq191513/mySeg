...
def capsule(u, op, k, s, t, z, routing):
    ...
    for u_t in u_t_list: # u_t: [N, H_0, W_0, z_0]
      if op == "conv":
        u_hat_t = conv2d(u_t, t_1*z_1, k, s,"SAME",True)
      elif op == "deconv":
        u_hat_t = conv2d_transpose(u_t, t_1*z_1, k, s,"SAME",True)
      else:
...
def residual_cap_block(input,routing,batchnorm_istraining=None):
    ...
    residul = input +cap_2
    if batchnorm_istraining is not None:
        # residul = bn(residul, batchnorm_istraining)
        residul = (input + cap_2)/2
    return residul
...
def my_segcap(images,is_train,size, l2_reg):
    ...
    # 1  (128 -> 128)
    conv1 =conv(images, filters=32, l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
    ...
    cap1_1 = residual_cap_block(conv_prime,routing=2,batchnorm_istraining=True)
    ...
    cap2_1 = residual_cap_block(cap1_2,routing=2,batchnorm_istraining=True)
    ...
    # cap3_1 = residual_cap_block(cap2_2,routing=2,batchnorm_istraining=True)
    # cap3_2 = capsule(cap3_1, "conv", k=3, s=2, t=start_s*multiple, z=32, routing=2)
    # skip3 = cap3_1

    #middle  (16 -> 16)
    multiple = 4
    cap_m_1 = capsule(cap2_2, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=2)
    # cap_m_2 = capsule(cap_m_1,"conv", k=3, s=1, t=start_s*multiple, z=32, routing=2)
    cap_m_2 = residual_cap_block(cap_m_1,routing=2,batchnorm_istraining=True)
    ...
    # 1/4  (32 -> 64)
    multiple = 2
    u_cap2_1 = capsule(cap_m_2, "deconv", k=3, s=2, t=start_s*multiple, z=32, routing=2)
    ...
    u_cap2_3 = residual_cap_block(u_cap_concat_2,routing=2,batchnorm_istraining=True)
    ...
    u_cap3_2 = residual_cap_block(u_cap_concat_3,routing=2,batchnorm_istraining=True)
    ...
    cap_out_1 = residual_cap_block(u_cap3_2,routing=2,batchnorm_istraining=True)
    # cap_out_2 = residual_cap_block(cap_out_1,routing=2,batchnorm_istraining=True)
    cap_out_3 = capsule(cap_out_1, "conv", k=3, s=1, t=1, z=1, routing=2)
