...
def bce_dice_margin_loss(y_true, y_pred,shape):
    y_true = tf.reshape(y_true,shape)
    y_pred = tf.reshape(y_pred,shape)
    batch_loss = binary_crossentropy(y_true, y_pred) + dice_loss(y_true, y_pred) + margin_loss(v_lens=y_pred,labels=y_true)
    mean_loss = tf.reduce_mean(tf.cast(batch_loss, tf.float32))
    return mean_loss
...
def bce_dice_margin_focus_loss(y_true, y_pred,shape):
    ...
    margin_loss(v_lens=y_pred,labels=y_true) + \
    weight_focus*focal_loss(y_true, y_pred, axis=[1])
    mean_loss = tf.reduce_mean(tf.cast(batch_loss, tf.float32))
    return mean_loss
...
def focal_loss(y_true, y_pred,axis=[1]):
    # gamma = 0.75
    gamma = 2
    alpha = 0.25
