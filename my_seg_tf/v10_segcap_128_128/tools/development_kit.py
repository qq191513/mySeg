...
def restore_model(sess,ckpt,restore_model):
    ...
    if restore_model:
        ...
        start_epoch = re.sub('\D','',model_name)
        start_epoch = int(start_epoch)+1
    return saver,start_epoch
...
def set_optimizer(lr_range,num_batches_per_epoch = None,loss = None):
    ...
    lrn_rate = tf.maximum(tf.train.exponential_decay(
        lr_start, global_step, num_batches_per_epoch,decay_factor), lr_end)
...
def print_effect_message(epoch_n,n_batch,n_batch_train,loss_value,dice_hard):
    # 显示训练效果
    message_1 = '\repoch_n=%d ' % epoch_n
    message_2 = 'n_batch={}/{} '.format(int(n_batch),n_batch_train-1)
    message_3 = 'loss=%0.3f ' % loss_value
    message_4 = 'dice_hard=%0.3f ' % dice_hard
...
def print_tensor(tensor,message=None):
	if message is None:
		message = 'Debug '
	return tf.Print(tensor, [tensor], message=message+': ', summarize=1000)
...
def show_parament_numbers():
    ...
    params = get_num_params()
    params = formatNum(params)
    print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxx parament numbers is : ' +params )
