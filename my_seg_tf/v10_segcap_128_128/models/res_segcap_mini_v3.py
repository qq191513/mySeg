...
def my_segcap(images,is_train,size, l2_reg):
    ...
    conv2 =conv(images, filters=atom, kernel_size=[3,3],l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
    prime_concat = tf.concat([conv1, conv2], axis=3)
    conv_prime = tf.expand_dims(prime_concat, axis=3)  # [N, H, W, t=1, z]
    # conv_prime = print_tensor(conv_prime,'conv_prime')
    skip0 = conv_prime
    # 1/2  (128 -> 64)
    multiple = 1
    # cap1_1 = capsule(conv_prime, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    cap1_1 = residual_cap_block(conv_prime,routing=3)
    # cap1_1 = print_tensor(cap1_1,'cap1_1')
    cap1_2 = capsule(cap1_1, "conv", k=3, s=2, t=start_s*multiple, z=atom, routing=3)
    # cap1_2 = print_tensor(cap1_2,'cap1_2')
    skip1 = cap1_1

    # 1/4  (64 -> 32)
    multiple = 2
    # cap2_1 = capsule(cap1_2, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    cap2_1 = residual_cap_block(cap1_2,routing=3)
    # cap2_1 = print_tensor(cap2_1,'cap2_1')
    cap2_2 = capsule(cap2_1, "conv", k=3, s=2, t=start_s*multiple, z=atom, routing=3)
    ...
    # 1/8  (32 -> 16)
    # multiple = 4
    # cap3_1 = capsule(cap2_2, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    # cap3_1 = residual_cap_block(cap2_2,routing=3,batchnorm_istraining=True)
    # cap3_2 = capsule(cap3_1, "conv", k=3, s=2, t=start_s*multiple, z=32, routing=3)
    # skip3 = cap3_1

    #middle  (16 -> 16)
    # multiple = 4
    # cap_m_1 = capsule(cap2_2, "conv", k=3, s=1, t=start_s*multiple, z=8, routing=3)
    # cap_m_1 = print_tensor(cap_m_1,'cap_m_1')
    # cap_m_2 = capsule(cap_m_1,"conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    cap_m_1 = residual_cap_block(cap2_2,routing=3)
    # cap_m_1 = print_tensor(cap_m_1,'cap_m_1')
    ...
    # 1/8  (16 -> 32)
    # multiple = 4
    # u_cap1_1 = capsule(cap_m_2, "deconv", k=3, s=2, t=start_s*multiple, z=32, routing=3)
    # u_cap_concat_1 = tf.concat([u_cap1_1, skip2], axis=3)
    # u_cap1_2 = capsule(u_cap_concat_1, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    # u_cap1_3 = capsule(u_cap1_2, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    # u_cap1_2 = residual_cap_block(u_cap_concat_1,routing=3,batchnorm_istraining=True)

    # 1/4  (32 -> 64)
    multiple = 2
    u_cap2_1 = capsule(cap_m_2, "deconv", k=3, s=2, t=start_s*multiple, z=atom, routing=3)
    ...
    # 1/2  (64 -> 128)
    multiple = 1
    u_cap3_1 = capsule(u_cap2_3, "deconv", k=3, s=2, t=start_s*multiple, z=atom*2, routing=3)
    ...
    # u_cap3_2 = capsule(u_cap_concat_3, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    u_cap3_2 = residual_cap_block(u_cap_concat_3,routing=3)
    # u_cap3_2 = print_tensor(u_cap3_2,'u_cap3_2')

    # 1   (128 -> 128)
    # cap_out_1 = capsule(u_cap3_2, "conv", k=3, s=1, t=start_s*multiple, z=32, routing=3)
    # cap_out_2 = capsule(cap_out_1, "conv", k=3, s=1, t=1, z=1, routing=3)
    cap_out_1 = residual_cap_block(u_cap3_2,routing=3)
    # cap_out_1 = print_tensor(cap_out_1,'cap_out_1')
    # cap_out_2 = residual_cap_block(cap_out_1,routing=3,batchnorm_istraining=True)

    ############new add ###########
    cap_out_3 = capsule(cap_out_1, "conv", k=3, s=1, t=1, z=8, routing=3)
    u_cap_concat_4 = tf.concat([cap_out_3, skip0], axis=4)

    cap_out_4 = tf.squeeze(u_cap_concat_4, axis=3)
    cap_out_5 =conv(cap_out_4, filters=256, kernel_size=[3,3],l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
    cap_out_6 =conv(cap_out_5, filters=128, kernel_size=[3,3],l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
    cap_out_7 =conv(cap_out_6, filters=1, kernel_size=[3,3],l2_reg_scale=l2_reg, batchnorm_istraining=is_training)
