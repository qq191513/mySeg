...
def my_segcap(images,is_train,size, l2_reg):
    ...
    print('conv_prime: {}'.format(conv_prime.get_shape()))
    skip0 = conv_prime
    print('skip0  conv_prime: {}'.format(skip0.get_shape()))
    # 1/2  (128 -> 64)
    ...
    skip1 = cap1_1
    print('skip1  residual_cap_block: {}'.format(skip1.get_shape()))
    # 1/4  (64 -> 32)
    multiple = 2
    cap2_1 = residual_cap_block(cap1_2,routing=routing)
    print('residual_cap_block: {}'.format(cap2_1.get_shape()))
    cap2_2 = capsule(cap2_1, "conv", k=3, s=2, t=start_s*multiple, z=atom, routing=routing)
    print('cap2_2: {}'.format(cap2_2.get_shape()))
    skip2 = cap2_1
    print('skip2  residual_cap_block: {}'.format(skip2.get_shape()))

    #middle  (16 -> 16)
    cap_m_1 = residual_cap_block(cap2_2,routing=routing)
    print('cap_m_1: {}'.format(cap_m_1.get_shape()))
    cap_m_2 = residual_cap_block(cap_m_1,routing=routing)
    print('cap_m_2: {}'.format(cap_m_2.get_shape()))
    # 1/4  (32 -> 64)
    multiple = 2
    u_cap2_1 = capsule(cap_m_2, "deconv", k=3, s=2, t=start_s*multiple, z=atom, routing=routing)
    print('u_cap2_1: {}'.format(u_cap2_1.get_shape()))
    u_cap_concat_2 = tf.concat([u_cap2_1, skip2], axis=3)
    print('u_cap_concat_2: {}'.format(u_cap_concat_2.get_shape()))
    u_cap2_3 = residual_cap_block(u_cap_concat_2,routing=routing)
    print('residual_cap_block: {}'.format(u_cap2_3.get_shape()))

    # 1/2  (64 -> 128)
    multiple = 1
    u_cap3_1 = capsule(u_cap2_3, "deconv", k=3, s=2, t=start_s*multiple, z=atom, routing=routing)
    print('u_cap3_1: {}'.format(u_cap3_1.get_shape()))
    u_cap_concat_3 = tf.concat([u_cap3_1, skip1], axis=3)
    print('u_cap_concat_3: {}'.format(u_cap_concat_3.get_shape()))
    u_cap3_2 = residual_cap_block(u_cap_concat_3,routing=routing)
    print('residual_cap_block: {}'.format(u_cap3_2.get_shape()))
    cap_out_1 = residual_cap_block(u_cap3_2,routing=routing)
    print('residual_cap_block: {}'.format(cap_out_1.get_shape()))

    # 1  (128 -> 128)
    cap_out_3 = capsule(cap_out_1, "conv", k=3, s=1, t=1, z=atom, routing=routing)
