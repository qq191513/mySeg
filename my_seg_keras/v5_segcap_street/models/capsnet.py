...
import sys
sys.path.append('../')
from models.capsule_layers import ConvCapsuleLayer, DeconvCapsuleLayer
from v1 import config as cfg


def CapsNetR3(input_shape):
    ...
    seg_caps = ConvCapsuleLayer(kernel_size=1, num_capsule=cfg.n_classes, num_atoms=1, strides=1, padding='same',
                                routings=3, name='seg_caps')(up_3)

    # seg_caps = K.squeeze(seg_caps,axis=-1)
    out_seg = layers.Reshape((cfg.output_shape[0] * cfg.output_shape[1], cfg.n_classes))(seg_caps)
    model = Model(inputs=[x], outputs=[out_seg])
    return model

#
# def CapsNetR1(input_shape):
...
#
#     # Layer 4: Convolutional Capsule: 1x1
#     seg_caps = ConvCapsuleLayer(kernel_size=1, num_capsule=1, num_atoms=3, strides=1, padding='same',
