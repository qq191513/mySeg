...
import sys
sys.path.append('../')
from models.capsule_layers import ConvCapsuleLayer, DeconvCapsuleLayer, Mask, Length
...
def CapsNetR3(input_shape, n_class=2, enable_decoder=True):
    ...
    deconv_cap_1_1 = DeconvCapsuleLayer(kernel_size=3, num_capsule=8, num_atoms=32, upsamp_type='deconv',
    ...
    deconv_cap_2_1 = DeconvCapsuleLayer(kernel_size=3, num_capsule=4, num_atoms=16, upsamp_type='deconv',
                                        scaling=2, padding='same', routings=3,
    ...
    deconv_cap_3_1 = DeconvCapsuleLayer(kernel_size=3, num_capsule=2, num_atoms=16, upsamp_type='deconv',
    ...
    # out_seg = layers.Reshape((cfg.output_shape[0] * cfg.output_shape[1], 1))(out_seg_2d)
    ...
    def shared_decoder(mask_layer):
        ...
        # out_recon = layers.Reshape((cfg.output_shape[0] * cfg.output_shape[1], 1))(out_recon)

        return out_recon

    # Models for training and evaluation (prediction)
    train_model = Model(inputs=[x,y], outputs=[out_seg_1,shared_decoder(masked_by_y)])
    if enable_decoder == True:
        eval_model = Model(inputs=x, outputs=[out_seg_1, shared_decoder(masked)])
    else:
        eval_model = Model(inputs=x, outputs=[out_seg_1])
